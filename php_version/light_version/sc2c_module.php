...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        readfile($filename);
        unlink($filename); // Remove o arquivo temporário
        exit;
    }
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#submitSubdomain').click(function() {
                ...
                // Armazena o subdomínio manual
                if (!manualSubdomains[currentRowIndex]) {
                    manualSubdomains[currentRowIndex] = {};
                }
                if (!manualSubdomains[currentRowIndex][program]) {
                    manualSubdomains[currentRowIndex][program] = [];
                }
            ...
            $('#downloadExcelBtn').click(function() {
                ...
                try {
                    ...
                    var approvalData = [
                        ["Programa", "Área", "Linha de Pesquisa", "Usuário", "Classificação Aprovada", "Score", "Data de Aprovação"]
                    ];
                    ...
                    $('tbody tr').each(function() {
                        ...
                        // Helper function to add approval data
                        function addApprovalData(type, index, classification) {
                            approvalData.push([
                                rowData.Programa,
                                rowData.Área,
                                rowData.Linha_de_Pesquisa,
                                userName,
                                type + ' ' + (index + 1) + ': ' + classification.code + ' - ' + classification.description,
                                classification.score.toFixed(3),
                                new Date().toLocaleString()
                            ]);
                        }
                        
                        // Process ACARE approvals
                        for (var i = 0; i < 3; i++) {
                            var acareKey = rowIndex + '-acare-' + i;
                            var classification;
                            
                            // Check manual subdomain selection first
                            if (manualSubdomains[rowIndex] && manualSubdomains[rowIndex]['ACARE'] && manualSubdomains[rowIndex]['ACARE'][i]) {
                                classification = manualSubdomains[rowIndex]['ACARE'][i];
                                if (classification && classification.code !== 'N/A') {
                                    addApprovalData('ACARE', i, classification);
                                }
                            } else if (approvals[acareKey] === 'Approved') {
                                classification = rowData.ACARE[i] || {code: 'N/A', description: 'Sem classificação', score: 0};
                                addApprovalData('ACARE', i, classification);
                            }
                        }
                        
                        // Process NASA approvals
                        for (var i = 0; i < 3; i++) {
                            var nasaKey = rowIndex + '-nasa-' + i;
                            var classification;
                            
                            // Check manual subdomain selection first
                            if (manualSubdomains[rowIndex] && manualSubdomains[rowIndex]['NASA'] && manualSubdomains[rowIndex]['NASA'][i]) {
                                classification = manualSubdomains[rowIndex]['NASA'][i];
                                if (classification && classification.code !== 'N/A') {
                                    addApprovalData('NASA', i, classification);
                                }
                            } else if (approvals[nasaKey] === 'Approved') {
                                classification = rowData.NASA[i] || {code: 'N/A', description: 'Sem classificação', score: 0};
                                addApprovalData('NASA', i, classification);
                            }
                        }
                    });
                    
                    // Create approvals worksheet with formatting
                    ...
                    // Apply formatting
                        ...
                        ws['!cols'] = [
                            ...
                            { wch: 15 }, // Usuário
                            { wch: 40 }, // Classificação Aprovada
                            { wch: 10 }, // Score
                            { wch: 20 }  // Data de Aprovação
                        ];

                        // Primeiro, garanta que todas as células existam
                        ...
                        // Formatação do cabeçalho (linha 0)
                    ...
                    for (var i = 1; i <= range.e.r; i++) {
                        for (var j = range.s.c; j <= range.e.c; j++) {
                            var cell = ws[XLSX.utils.encode_cell({r: i, c: j})];
                            if (cell) {
                                cell.s = {
                                    fill: { fgColor: { rgb: i % 2 === 0 ? "F5F6F5" : "FFFFFF" } },
                                    border: {
                                        top: { style: "thin" },
                                        bottom: { style: "thin" },
                                        left: { style: "thin" },
            ...
            $('#downloadBtn').click(function() {
                ...
                $('tbody tr').each(function() {
                    ...
                    if (hasApprovals) {
                        ...
                        // Adiciona as classificações ACARE e NASA, incluindo as manuais
                        var acareClassifications = [];
                        var nasaClassifications = [];
                        
                        // Processa ACARE
                        if (manualSubdomains[rowIndex] && manualSubdomains[rowIndex]['ACARE']) {
                            acareClassifications = manualSubdomains[rowIndex]['ACARE'];
                        } else {
                            acareClassifications = rowData.ACARE.slice(0, 3);
                        }
                        
                        // Processa NASA
                        if (manualSubdomains[rowIndex] && manualSubdomains[rowIndex]['NASA']) {
                            nasaClassifications = manualSubdomains[rowIndex]['NASA'];
                        } else {
                            nasaClassifications = rowData.NASA.slice(0, 3);
                        }
                        
                        // Preenche até 3 classificações para cada
                        for (var i = 0; i < 3; i++) {
                            if (i < acareClassifications.length && acareClassifications[i] && acareClassifications[i].code !== 'N/A') {
                                csvContent += '"' + acareClassifications[i].code + ': ' + acareClassifications[i].description + 
                                    ' (' + acareClassifications[i].score.toFixed(3) + ')",';
                            } else {
                                csvContent += '"",';
                            }
                        }
                        ...
                        for (var i = 0; i < 3; i++) {
                            if (i < nasaClassifications.length && nasaClassifications[i] && nasaClassifications[i].code !== 'N/A') {
                                csvContent += '"' + nasaClassifications[i].code + ': ' + nasaClassifications[i].description + 
                                    ' (' + nasaClassifications[i].score.toFixed(3) + ')",';
                            } else {
                                csvContent += '"",';
                            }
                        }
        ...
        function createClassificationDiv(classification, type, index) {
            return $('<div class="classification" data-type="' + type.toLowerCase() + '" data-index="' + index + '">').html(
                ...
                '</div>' +
                '<div class="chart-container">' +
                '<canvas id="' + type.toLowerCase() + 'Chart-' + index + '"></canvas>' +
                '</div>'
            );
        }
    </script>
