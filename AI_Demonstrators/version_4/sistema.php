<?php
    ...
    // Construir clausulas de filtro com prepared statements
    if (!empty($searchQuery)) {
        $searchParam = '%' . $searchQuery . '%';
        $filterClauses[] = "(p.idProj LIKE ? OR p.pName LIKE ? OR p.resp LIKE ? OR p.idProj IN (SELECT pc.project_id FROM project_category pc JOIN category c ON pc.category_id = c.idCat WHERE c.Category LIKE ?) OR p.id_TRL IN (SELECT t.idTRL FROM trl t WHERE t.TRL LIKE ?))";
        
        // Adicionar o mesmo parametro 5 vezes para cada LIKE
        for($i = 0; $i < 5; $i++) {
            $params[] = $searchParam;
            $types .= "s";
        }
    ...
    // Combinar clausulas de filtro
    $filterClause = "";
    if (!empty($filterClauses)) {
        $filterClause = "AND (" . implode(" AND ", $filterClauses) . ")";
    }

    $countSql = "SELECT COUNT(*) as total FROM project p WHERE p.sit_Project_id IN ($idSituationString) $filterClause";

    // Query para contar registros
    $countStmt = $link->prepare($countSql);
    if (!empty($params)) {
        // Cria um array com os tipos como primeiro elemento
        $bindParams = array($types);
        // Adiciona referências dos parâmetros
        foreach ($params as &$param) {
            $bindParams[] = &$param;
        }
        // Chama bind_param usando call_user_func_array
        call_user_func_array(array($countStmt, 'bind_param'), $bindParams);
    }
    $countStmt->execute();
    
    // Usar bind_result() em vez de get_result()
    $totalRecords = 0;
    ...
    $sql = "SELECT p.idProj, p.pName, p.id_TRL, p.id_Laboratory, p.descr, p.link_ref, p.email, p.resp, p.sit_Project_id, p.date_in FROM project p WHERE p.sit_Project_id IN ($idSituationString) $filterClause ORDER BY p.pName ASC LIMIT ?, ?";    
    $stmt = $link->prepare($sql);
    
    // Adicionar parametros de paginacao
    $allParams = $params;
    $allParams[] = $offset;
    ...
    // Usar bind_result() em vez de get_result()
    $stmt->bind_result($idProj, $pName, $id_TRL, $id_Laboratory, $descr, $link_ref, $email, $resp, $sit_Project_id, $date_in);
